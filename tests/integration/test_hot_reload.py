...
# 热重载是可选功能，watchdog 只在 requirements.dev.txt 中（见 P1-8）。
# 未安装时整个模块跳过，而不是让收集期报 ImportError。
pytest.importorskip("watchdog", reason="配置热重载依赖 watchdog（requirements.dev.txt）")
