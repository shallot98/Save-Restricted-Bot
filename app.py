...
def media(storage_location):
    """提供媒体文件访问（支持本地和 WebDAV，支持 Range 请求）"""
    if 'username' not in session:
        return redirect(url_for('login'))
        ...
        if file_path_or_url.startswith('http://') or file_path_or_url.startswith('https://'):
            try:
                ...
                if response.status_code in (200, 206):
                    ...
                    # 添加 Content-Length 和 Content-Range（如果有）
                ...
                if range_header:
                    ...
                    if match:
                        ...
                        response = Response(
                            data,
                            status=206,
                            mimetype=_get_mimetype(file_path_or_url)
                        )
...
def api_calibrate(note_id):
    ...
    try:
        ...
        all_dns = extract_all_dns_from_note(note)
        if not all_dns:
            return jsonify({'success': False, 'error': '没有找到磁力链接'}), 404
        ...
        if update_note_with_calibrated_dns(note_id, calibrated_results):
            ...
            return jsonify({'success': False, 'error': '更新数据库失败'}), 500

    except Exception as e:
        import traceback
        print(f"[ERROR] 校准API异常: {e}")
        traceback.print_exc()
