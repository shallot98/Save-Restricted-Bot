...
> AI may help run `git commit` only if:
> 1. this checklist has passed, and
> 2. the user explicitly asks or confirms that AI should commit.
...
- [ ] If AI will commit: user explicitly asked/confirmed that AI should commit
- [ ] Commit scope reviewed (avoid unrelated dirty files)
- [ ] Commit message prepared in `type(scope): description` format
...
Development Flow:
  Write code -> Test -> $finish-work -> git commit -> $record-session
                          |               |
                   Ensure completeness    Human by default, or AI with explicit user confirmation
                                                          |
                                                   Record progress
